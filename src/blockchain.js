...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async(resolve, reject) => {
            ...
            if (messageTime + 300 < currentTime) {
                reject('The message is not valid, time has elapsed');
            } else {
    ...
    getStarsByWalletAddress(address) {
        ...
        self.chain.forEach(block => {
            //decode each block and check if the address is the same
            let decodedData = JSON.parse(hex2ascii(block.body));
            if (decodedData.data.address) {
                if (decodedData.data.address === address) {
                    let star = decodedData.data.star
    ...
    validateChain() {
        ...
        let errorLog = [];
        return new Promise(async(resolve, reject) => {
            this.chain.forEach(async(p, i) => {
                try {
                    let block = await BlockClass.validate(p, i);
                    if (i > 0) {
                        if (block.previousBlockHash !== self.chain[i - 1].hash) {
                            errorLog.push(`Block #${block.height} Previous hash does not match`);
                        }
                    }
                } catch (error) {
                    errorLog.push(error);
